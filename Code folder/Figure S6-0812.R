...
pred_mod <- ggeffect(mod, terms = c("Effect_size_com"))
eff_mod_data <- data.frame(pred_mod)
colnames(eff_mod_data)[1] <- "Effect_size_com"
colnames(eff_mod_data)[2] <- "Effect_size_all"
...
ggplot(Field_group, aes(Effect_size_com, Effect_size_all))+
  ...
  geom_line(data = eff_mod_data, mapping = aes(Effect_size_com, Effect_size_all), size = 1.2) +
  scale_x_continuous(labels = scales::label_comma(accuracy =0.01), limits = c(-0.8,0.42)) +
  scale_y_continuous(labels = scales::label_comma(accuracy =0.01), limits = c(-0.8,0.42)) + 
  theme_bw() + mytheme + 
  ...
  annotate("text", x = 0, y = -0.31, label = "F[1*','*377.87] == 1839.8", parse = TRUE, size = 4) + 
  annotate("text", x = 0, y = -0.38, label = "italic(R)^2*m == 0.744", parse = TRUE, size = 4) + 
...
# 
drop1(fm1, test = "Chi") 
f.sbs1 <- update(fm1, ~. -Phylo_Di_log:Soil_ph)
drop1(f.sbs1, test = "Chi") 
f.sbs2 <- update(f.sbs1, ~. -Funct_Di_log:Tave_site)
drop1(f.sbs2, test = "Chi") 
f.sbs3 <- update(f.sbs2, ~. -Phylo_Di_log:Precip_site)
drop1(f.sbs3, test = "Chi") 
f.sbs4 <- update(f.sbs3, ~. -Funct_Di_log:Precip_dev)
drop1(f.sbs4, test = "Chi") 
f.sbs5 <- update(f.sbs4, ~. -Phylo_Di_log:Wcont)
drop1(f.sbs5, test = "Chi") 
f.sbs6 <- update(f.sbs5, ~. -Phylo_Di_log:Soil_N)
drop1(f.sbs6, test = "Chi") 
f.sbs7 <- update(f.sbs6, ~. -Phylo_Di_log:Precip_dev)
drop1(f.sbs7, test = "Chi") 
f.sbs8 <- update(f.sbs7, ~. -Funct_Di_log:Precip_site)
drop1(f.sbs8, test = "Chi") 
f.sbs9 <- update(f.sbs8, ~. -Precip_site)
drop1(f.sbs9, test = "Chi") 
f.sbs10 <- update(f.sbs9, ~. -Phylo_Di_log:Tave_dev)
drop1(f.sbs10, test = "Chi") 
f.sbs11 <- update(f.sbs10, ~. -Funct_Di_log:Soil_ph)
drop1(f.sbs11, test = "Chi") 
f.sbs12 <- update(f.sbs11, ~. -Soil_ph)
drop1(f.sbs12, test = "Chi") 
f.sbs13 <- update(f.sbs12, ~. -Precip_dev)
drop1(f.sbs13, test = "Chi") 
f.sbs14 <- update(f.sbs13, ~. -Tave_year)
drop1(f.sbs14, test = "Chi") 
f.sbs15 <- update(f.sbs14, ~. -Funct_Di_log:Wcont)
drop1(f.sbs15, test = "Chi") 
f.sbs16 <- update(f.sbs15, ~. -Wcont)
drop1(f.sbs16, test = "Chi") 
f.sbs17 <- update(f.sbs16, ~. -Funct_Di_log:Tave_dev)
drop1(f.sbs17, test = "Chi") 
f.sbs18 <- update(f.sbs17, ~. -Tave_dev)
drop1(f.sbs18, test = "Chi") 
f.sbs19 <- update(f.sbs18, ~. -Precip_year)
...
Table_S4 <- as.data.frame(anova(fm1_test, ddf = "Kenward-Roger"))
...
Table_S4$VIF <- round(car::vif(fm1_test), 2)

################################## Figure S6a ###################################
# Obtaining standardized regression coefficients and their 95% CI
Final_model = lmer(Effect_size ~ PCoA1 + Tave_site + Soil_N + Phylo_Di_log + Funct_Di_log + 
...
MegaModelSummary_all$Group <- c("Field com", rep("Climate", 1), rep("Soil properties", 1), 
...
#ggsave("Figure_S6b.pdf", plot = Figure_S6b, width = 9.11, height = 10.10, units = "in", dpi = 300)
