...
for (Y in Years_names) {
  ...
  for (L in Latitude_names) {
    ...
    result <- sample_group2 %>% dplyr::group_by(Species) %>% dplyr::summarize(count = n()) %>% filter(count > 1)
...
#write.xlsx(Final_total_distinct, "Final_total_distinct_all.xlsx")
...
# Simpson distance matrix for shared taxa in field
Shared_Field_otu_01 <- t(Field_otu_raw[common_ASVs,])
Shared_Field_otu_01[Shared_Field_otu_01 > 0] = 1 
...
Shared_Green_otu_01[Shared_Green_otu_01 > 0] <- 1 
...
Green_dist <- as.matrix(Sim_dist_green_Shared) 
Green_dist_data <- reshape2::melt(Green_dist, varnames = c("Sample_ID_A", "Sample_ID_B"),
                                  value.name = "dist", na.rm = T)
colnames(Green_dist_data)[1] <- "Sample_ID"
Green_dist_data <- Green_dist_data %>% left_join(Green_group[,c("Sample_ID","Species")], by = "Sample_ID")
...
# Input Simpson distance matrix of fungi
Field_Fungal_dist <- as.matrix(Sim_dist_field_Shared)
Green_Fungal_dist <- as.matrix(Sim_dist_green_mean)
...
# save the results
head(Final_total_distinct)
#write.xlsx(Final_total_distinct, "Final_total_distinct_shared.xlsx")
